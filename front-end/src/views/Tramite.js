...
export default class Tramite extends Component {
    ...
    getPasos = (id) => {
        ...
        API.restCall({
            service:'get_pasos_proceso/' + id,
            method:'get',
            success:(response) => {this.setState({ pasos: response, loading:false, status:this.state.n_paso==response.length ? "Terminado" : this.state.n_paso == 1 ? "Iniciado":"En proceso" });
    ...
    refreshData = () => {
        ...
        API.restCall({
            service:'get_tramite/' + this.props.id ,
            success:(response) => {
    ...
    render() {
        return (
            <Spin spinning={this.state.loading}><div>
                <h2 style={{float:'right'}}>{ localStorage.getItem("esAdmin") ? <Statistic title="Matrícula" groupSeparator={""} value={(localStorage.getItem("matAlumno") || " ").toLocaleUpperCase()} />: "" }</h2>
                ...
                <MediaQuery query="(min-device-width: 1224px)"><Steps labelPlacement={'vertical'} current={this.state.n_paso - 1} style={{marginBottom:50}}>{this.state.pasos.map(value => {return (<Steps.Step title={value.nombre} />)})}</Steps></MediaQuery>
                ...
                <Row gutter={8}>
                    ...
                    <Col span={12}><Row gutter={8} style={{textAlign:'center'}}>
                        <Col span={12}><Statistic title="Fecha de inicio" value={moment.utc(this.state.fecha1).format("DD/MM/YYYY")} prefix={<Icon type="calendar" />} /></Col>
                        <Col span={12}><Statistic title="Fecha de última actualización" value={moment.utc(this.state.fecha2).format("DD/MM/YYYY")} prefix={<Icon type="calendar" />}/></Col>
                        </Row>
                    </Col>
                </Row>
            </div>
