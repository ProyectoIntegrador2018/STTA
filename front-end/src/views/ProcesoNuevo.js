    ...
    render() {
        return (
            <div>
                ...
                <Upload.Dragger beforeUpload={(file, fileList) => {this.parseFile(file);return false;}}>
                    <p className="ant-upload-drag-icon"> <Icon type="upload" /> </p>
                ...
                <Form.Item label="Columna de la matrícula: ">
                    <Select style={{ width: '100%' }} onChange={(value) => this.handleSelect('matricula',value)}>
                        {this.state.columns.map((element) => (<Select.Option value={element.key}>{element.title}</Select.Option>))}
                    </Select>
                </Form.Item>
                    </Col>
                    <Col span={12}>
                ...
                <Form.Item label="Columnas de pasos: ">
                            ...
                            title={
                                ...
                                <Col span={10}>
                                    <Form.Item label="Columna: ">
                                        <Select style={{ width: '100%', marginRight: 15 }} onChange={(value) => this.edit(index,'columna_csv',value)}>
                                            {this.state.columns.map((element) => ( <Select.Option value={element.key}>{element.title}</Select.Option> ))}
                                        </Select>
                                    </Form.Item>
                                </Col>
                                <Col span={10}>
                                    <Form.Item label="Nombre a mostrar: ">
                                        <Input placeholder="Paso" style={{ width: '100%', marginRight: 15 }} onChange={(e) => this.edit(index,'nombre_mostrar',e.target.value)}/>
                                    </Form.Item>
                                </Col>
                                <Col span={1}>
                                    <Form.Item label=" " colon={false}>
                                        <Button type={'danger'} disabled={this.state.pasos.length - 1 !== index} hidden={this.state.pasos.length - 1 !== index} onClick={() => this.remove()} icon={"minus-circle-o"}/>
                                    </Form.Item>
                                </Col>
                                </Row>
                    ...
                    <div style={{textAlign:'center'}}>
                        <Button type="dashed" onClick={this.add} style={{ width: '60%' }}> <Icon type="plus" /> Agregar paso </Button>
                    </div>
                </Form.Item>
