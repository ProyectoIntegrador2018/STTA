    ...
    render() {
        return (
            <div><h2>Proceso nuevo</h2>
                ...
                <Upload.Dragger beforeUpload={(file, fileList) => {this.parseFile(file);return false;}}><p className="ant-upload-drag-icon"> <Icon type="upload" /> </p><p className="ant-upload-text">Haz clic o arrastra un documento en esta área</p><p className="ant-upload-hint">El sistema solo soporta archivos CSV</p></Upload.Dragger>
                <Divider/><h2>Selección de columnas</h2>
                <Row gutter={8}>
                    <Col span={12}><Form.Item label="Columna de la matrícula: "><Select style={{ width: '100%' }} onChange={(value) => this.handleSelect('matricula',value)}>{this.state.columns.map((element) => (<Select.Option value={element.key}>{element.title}</Select.Option>))}</Select></Form.Item></Col>
                    <Col span={12}><Form.Item label="Columna de # de ticket: "><Select style={{ width: '100%' }} onChange={(value) => this.handleSelect('ticket',value)}>{this.state.columns.map((element) => (<Select.Option value={element.key}>{element.title}</Select.Option>))}</Select></Form.Item></Col>
                </Row>
                <Row gutter={8}>
                    <Col span={12}><Form.Item label="Columna de fecha de apertura: "><Select style={{ width: '100%' }} onChange={(value) => this.handleSelect('fecha_apertura',value)}>{this.state.columns.map((element) => (<Select.Option value={element.key}>{element.title}</Select.Option>))}</Select></Form.Item></Col>
                    <Col span={12}><Form.Item label="Columna de fecha de última actualización:"><Select style={{ width: '100%' }} onChange={(value) => this.handleSelect('ultima_actualizacion',value)}>{this.state.columns.map((element) => (<Select.Option value={element.key}>{element.title}</Select.Option>))}</Select></Form.Item></Col>
                </Row>
                ...
                <Form.Item label="Columnas de pasos: "><Steps direction="vertical" current={1} style={{ width: '100%' }}>
                        {this.state.pasos.map((k, index) => (<Steps.Step key={index} status={'process'}  style={{ width: '100%' }}
                            title={
                                ...
                                <Col span={10}><Form.Item label="Columna: "><Select style={{ width: '100%', marginRight: 15 }} onChange={(value) => this.edit(index,'columna_csv',value)}>{this.state.columns.map((element) => ( <Select.Option value={element.key}>{element.title}</Select.Option> ))}</Select></Form.Item></Col>
                                <Col span={10}><Form.Item label="Nombre a mostrar: "><Input placeholder="Paso" style={{ width: '100%', marginRight: 15 }} onChange={(e) => this.edit(index,'nombre_mostrar',e.target.value)}/></Form.Item></Col>
                ...
                <div style={{textAlign:'right'}}><Button loading={this.state.loading} disabled={this.state.loading} onClick={this.uploadData} style={{marginTop:10}} className={'button-success'}><Icon type={'upload'}/> Subir datos</Button></div>
            </div>
        );
    }
}
