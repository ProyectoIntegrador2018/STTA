...
export default class Documentos extends Component {
    ...
    render() {
        ...
        if (localStorage.getItem("tipo") == '2') {
            botonsubir = <Link to={'/documentos/subir'}><Button style={{float:'right'}} type="secondary" icon="upload" disabled>Subir documento de proceso</Button></Link>
            permitirBorrar=false
        }
        else {
            botonsubir = <Link to={'/documentos/subir'}><Button style={{float:'right'}} type="secondary" icon="upload">Subir documento de proceso</Button></Link>
            permitirBorrar=true
        }
        return (
            <div>
